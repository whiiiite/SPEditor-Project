...
let currLine;
let currSection;
let selectedLang = 'sql';
let eventContainer;
const lineClass = '.line';
const lineFocusedClass = 'line-focused';
const sectionClass = 'section';
const lineClassName = 'line';
const lineNumberClass = 'line-number';
...
function applySyntaxLine(line) {
    analizeAndApplySyntax(selectedLang, line);
}
function findCurrentSection(globalPos) {
...
function setFocusEventElement(element, index) {
    const focusHandler = (event) => {
        ...
        currSection = currLine.lastChild;
        setCaretToPosition(getCaretClientPos());
    };
...
*/
function addLine(currLine, currentCharPos) {
...
function recountLines() {
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        lines[index].style.top = lineTop + 'px';
        lines[index].tabIndex = index + 1;
...
function enter(currentLine, currentSection, currPos) {
    ...
    if (!stringIsNullOrWhiteSpace(rightPart)) {
        ...
        if (addedLineData.isBefore) {
            currentSection = currentLine.firstChild;
        }
        else {
            currentSection = addedLine.firstChild;
        }
...
function backSpace(currLine, currIndex, currSection, currentPos) {
    const prevSection = currSection.previousSibling;
    const nextSection = currSection.nextSibling;
...
function setCursorTo(element, positionTo) {
...
function switchCaretVisibility() {
...
function caretUpdater(delay) {
    if (delay <= 0)
        throw new Error('delay must be more or equals than zero');
    setInterval(() => {
...
function caretVisibilitySwitcher(delay) {
    if (delay <= 0)
        throw new Error('delay must be more or equals than zero');
    setInterval(() => {
        const pos = getCaretClientPos();
...
// init editor
function __init_1() {
    currLine = document.getElementsByClassName(lineClassName)[0];
    currIndex = 0;
    currLine.focus();
    setKeyboardEvents();
    setFocusEvent();
